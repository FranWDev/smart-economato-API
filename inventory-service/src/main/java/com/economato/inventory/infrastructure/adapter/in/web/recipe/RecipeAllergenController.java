...
import com.economato.inventory.application.usecase.recipe.RecipeAllergenService;
...
public class RecipeAllergenController {
    ...
    @GetMapping
    @Operation(summary = "Obtener todas las relaciones receta-alérgeno", description = "Devuelve una lista paginada de todas las asociaciones entre recetas y alérgenos. [Rol requerido: USER]")
    @ApiResponse(responseCode = "200", description = "Lista de relaciones obtenida correctamente", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RecipeAllergen.class)))
    ...
    @PostMapping
    @Operation(summary = "Crear una nueva relación receta-alérgeno", description = "Asocia un alérgeno existente a una receta. [Rol requerido: CHEF]")
    @ApiResponses({
    ...
    @Operation(summary = "Obtener alérgenos por receta", description = "Devuelve todos los alérgenos asociados a una receta específica. [Rol requerido: USER]")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Lista de alérgenos asociados a la receta", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RecipeAllergen.class))),
            @ApiResponse(responseCode = "404", description = "Receta no encontrada")
    })
    public ResponseEntity<List<RecipeAllergen>> getByRecipe(
            @Parameter(description = "ID de la receta", example = "12", required = true) @PathVariable Integer recipeId) {
    ...
    @Operation(summary = "Obtener recetas por alérgeno", description = "Devuelve todas las recetas que contienen el alérgeno especificado. [Rol requerido: USER]")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Lista de relaciones encontradas", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RecipeAllergen.class))),
            @ApiResponse(responseCode = "404", description = "Alérgeno no encontrado")
    })
    public ResponseEntity<List<RecipeAllergen>> getByAllergen(
            @Parameter(description = "ID del alérgeno", example = "5", required = true) @PathVariable Integer allergenId) {
    ...
    @Operation(summary = "Asociar un alérgeno a una receta", description = "Crea una asociación entre una receta y un alérgeno existentes. [Rol requerido: CHEF]")
    @ApiResponses({
    ...
    @Operation(summary = "Eliminar asociación entre receta y alérgeno", description = "Elimina la asociación entre una receta específica y un alérgeno. [Rol requerido: CHEF]")
    @ApiResponses({
    ...
    @Operation(summary = "Obtener alérgenos de una receta", description = "Devuelve la lista de todos los alérgenos asociados a una receta. [Rol requerido: USER]")
    @ApiResponses({
